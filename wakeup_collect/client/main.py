...
def download_audio_file(ip_addr):
    global g_recorder_id, g_recorder_age, g_recorder_gender, g_recorder_province
    ...
    for i in range(1,16):
        num = "{0:04d}".format(i)
        filename = "{}_{}_{}_{}_{}_{}.wav".format(prefix , g_recorder_id ,g_recorder_gender,g_recorder_age , g_recorder_province, num)
        # print(filename)
        
        target = "http://{}/cgi-bin/output/{}".format(ip_addr, filename)
        print(target)
        try:
            r = requests.get(url=target, stream=False)
            local_file="./audio_files/"+filename
    ...
    if ip_addr == g_ipaddr_one:
        g_down_finish_one = True
    if ip_addr == g_ipaddr_three:
        g_down_finish_three = True
    if ip_addr == g_ipaddr_five:
        g_down_finish_five = True
    print("the end of donload all file")
...
class MainWindow(wx.Frame):
    def __init__(self):
        ...
        wx.Frame.__init__(
            self, None, title='DOSS语料采集工具', size=(MAX_WIDTH, 600))
        self.SetMaxSize((MAX_WIDTH,600))
        self.ui_main()
        self.Show(True)
    ...
    def ui_bottom(self):
        ...
        self.btnDownload = wx.Button(self, wx.NewId(), '下载设备上的录音文件')
    ...
    def onBtnDownload(self, event):
        ...
        t3.start()
        while True:
            if g_down_finish_one and g_down_finish_three and g_down_finish_five:
                break
            else:
                self.statusText.SetValue("下载中{}".format(time.asctime( time.localtime(time.time()) )))
                time.sleep(1)
        ...
        try:
            ...
            return 
        self.statusText.SetValue("删除板端文件完成，可以填入信息开始写一个录音采集")
    ...
    def onBtnFillInfo(self, event):
        ...
        global g_recorder_id, g_recorder_age, g_recorder_gender, g_recorder_province, g_recorder_time
        g_recorder_id = id
        g_recorder_age = age
